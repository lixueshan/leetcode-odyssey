...
后序遍历要求树的节点以“左右根”的顺序输出，前序遍历的输出顺序是“根左右”，稍加改动即可实现左右镜像的前序遍历，即按“根右左”顺序输出(只需将程序中的left改成right，right改成左)。在返回res前执行一行列表反转语句`Collections.reverse(res);`即可得到后序遍历的结果。

需要注意的是，在遍历过程中并不是“后序”，而是如上所说的左右镜像的前序遍历，只是结果符合后序。当强调“遍历”时，该方法并不符合要求，若只要求结果是后序的而不论过程如何，则可以使用该方法。编程角度的优点是可以套用前序遍历代码，易于理解，且无需一个反转右孩子链条的方法adjustRes。
