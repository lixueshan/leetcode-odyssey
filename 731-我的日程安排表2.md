...
容易看出日程安排的过程为区间问题。安排新日程之前需查询该日程区间内是否已存在二重预定。可设计为一区间求最值操作，即判断 $max(start, end - 1)$ 是否为小于 2 。是则可安排该日程，执行区间修改，采用增量式 $add(start,end-1,1)$ ，返回 $true$ 。否则返回 $false$ 。
