...
[668. 乘法表中第k小的数](https://leetcode.cn/problems/kth-smallest-number-in-multiplication-table/) (困难)。两个大步骤。

1. 看出是二分问题（需要一点经验）。
2. 找「循环不变」，正向提取出左右界的更新条件。再按图索骥找能支持我们写出该条件的量。
...
- 取得每次二分要考察的数 c = (l + r) / 2 之后（不一定在矩阵上），要使得「循环不变」为（按「模版一」写）： 
  ...
  - r = c - 1，使得 r 更新后，必满足 **「大于等于k个数」** 小于c。
  - 最终 l 就是「第k大」的数。

所以这其实是二分查找的 **「小于」情形** 。为了写出更新 l 和 r 的条件，需反推要如何知道对于c，矩阵中有多少个小于它的数。

写出几行矩阵，观察一下，可以有如下分析。
1. c 在第c/n行，前面的行中所有数均小于c。
2. 从c/n行开始，一直到m行，都可能存在小于c的数，累计即可。第k行小于c的数的个数为c/k。
...
[462. 最少移动次数使数组元素相等 II](https://leetcode.cn/problems/minimum-moves-to-equal-array-elements-ii/) (中等)。本题其实可以看作是[162. 寻找峰值 ](https://leetcode.cn/problems/find-peak-element/)的拓展题，思想一致，但本题稍难。如果你还没做过162，建议先做162再做本题。下面是解题时间。
...
4. 因为我们求的目标值 t 一定使得 t 对于nums的差分总值最小。如果从min到max的所有值都求出对应的差分总值，画出来就是一条U型的先降后升曲线。那么：
...
class Solution {
    ...
    private int diffCount(int[] nums, int target){
        ...
        for(int num : nums){
            ...
            // count += num > target ? num - target : target - num; // 会超时。。。谁能告诉我这是为啥？？
        }
        return count;
    }
}
...
[436. 寻找右区间](https://leetcode.cn/problems/find-right-interval/) (中等)。很容易看出如下做法，对所有区间的左界排序，遍历intervals，对每一个右界，用「二分查找」的「大于等于」情形，在排序的左界中寻找「大于等于」该右界的左界，将其所属区间在intervals中的下标返回到结果中。



可以用一维数组保存并排序左界，用map来保存左界和其所属区间在intervals中的下标，以便二分查找到该左界时相应下标。我们也可以不用map，而是用二维数组`int[][] lefts[intervals.length][2]`来保存左界。具体过程。
...
3. 遍历intervals，对每一个区间右界，以其为 target，在lefts中用二分查找，找到「大于等于」该target的左界值，不存在时返回 -1，存在则返回相应的下标。
