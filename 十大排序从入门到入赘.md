...
2.  遍历arr，每读取一个$arr[i]，直接令count$arr[$arr[i]]++。
...
> 工具推荐：
> (1) 本文在typora中完成书写。[typora](https://typoraio.cn/)，markdown书写工具。
> (2) 本文所有动图来自均来自[visualgo](https://visualgo.net/en/sorting)网站，一个非常著名的算法可视化网站，不仅限于排序，还包括「树」、「链表」、「堆」、「图」、「并查集」等等的可视化。。
> (3) 本文所有gif动图虽来自上述网站，但皆经由本地「[Kap](https://getkap.co/)」工具制作。
> (4) 本文所有公式均借助「[Mathpix Snipping Tool](https://mathpix.com/)」写出。
